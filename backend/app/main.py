...
async def debug_seed():
    ...
    log("Starting manual seed...")
    try:
        async with AsyncSessionLocal() as db:
            log("Checking for 'demo' tenant...")
            result = await db.execute(select(models.Tenant).where(models.Tenant.slug == "demo"))
            tenant = result.scalars().first()
            if not tenant:
                ...
                db.add(tenant)
                await db.flush()
                log(f"Created 'demo' tenant: {tenant.id}")
            else:
                log(f"Found 'demo' tenant: {tenant.id}")
            ...
            if not user:
                ...
                log(f"Found 'admin' user: {user.id}")
                
            await db.commit()
            log("Seeding committed.")
