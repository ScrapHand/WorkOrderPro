...
export default function TenantLoginPage({ params }: { params: Promise<{ tenantSlug: string }> }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.log("Attempting login to:", tokenUrl, "for tenant:", tenantSlug);
            setDebugInfo(`Calling: ${tokenUrl} (Tenant: ${tenantSlug})`);

            // 3. Prepare FormData (Standard FastAPI OAuth2)
            const formData = new FormData();
            formData.append("username", email);
            formData.append("password", password);
            // Optionally append tenant_slug to body if backend supports it there too, 
            // but Header is the primary method.
        ...
        <div className="flex min-h-screen items-center justify-center bg-muted/40 px-4">
            <Card className="w-full max-w-sm">
                <CardHeader className="space-y-1">
                    <CardTitle className="text-2xl font-bold">Sign in to {tenantSlug}</CardTitle>
                    <CardDescription>
