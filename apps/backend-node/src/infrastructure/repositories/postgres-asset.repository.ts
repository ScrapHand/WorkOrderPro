...
const mapToDomain = (row: any): Asset => {
    return new Asset(
        row.id,
        row.tenant_id || row.tenantId, // Handle both raw snake_case and prisma camelCase
        row.name,
    ...
    async findSubtree(rootId: string, tenantId: string): Promise<Asset[]> {
        ...
        const rawResults: any[] = await this.prisma.$queryRaw`
            WITH RECURSIVE asset_tree AS (
                -- Base Case: The Root
                SELECT id, tenant_id, parent_id, name, status, criticality, hierarchy_path, "description", "createdAt", "updatedAt", "deletedAt", 0 as depth
                FROM "Asset"
                ...
                SELECT child.id, child.tenant_id, child.parent_id, child.name, child.status, child.criticality, child.hierarchy_path, child."description", child."createdAt", child."updatedAt", child."deletedAt", parent.depth + 1
                FROM "Asset" child
    ...
    async findAncestors(assetId: string, tenantId: string): Promise<Asset[]> {
        ...
        const rawResults: any[] = await this.prisma.$queryRaw`
            WITH RECURSIVE ancestor_tree AS (
                ...
                SELECT parent.id, parent.tenant_id, parent.parent_id, parent.name, parent.status, parent.criticality, parent.hierarchy_path, parent."description", parent."createdAt", parent."updatedAt", parent."deletedAt", child.depth + 1
                FROM "Asset" parent
